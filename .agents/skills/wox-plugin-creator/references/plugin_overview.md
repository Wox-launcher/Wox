...
3. **Implement**:
   ...
   - Result rows use title, subtitle, and at most three tails. A fourth tail tag can be clipped. Add a preview only for a large body. Refresh a visible row with `UpdateResult` / `update_result`. Capsule tail metrics are in `SKILL.md`.
