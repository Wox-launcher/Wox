...
2. `launcher/theme.go` resolves launcher theme values into the portable palette. `launcher/settings_theme.go` independently owns the single Glass-derived dark Settings palette, with an opaque window background on Linux regardless of compositor blur support, a translucent window tint on macOS/Windows, and translucent popup tints over floating blur materials. Settings does not inherit launcher colors, geometry overrides, or system light/dark changes.
...
| Action Panel header | 12 regular type, one step below 13 action labels; 18 optically centered line; do not use a 16 Text slot |
...
Launcher outer chrome exposes `AppBorderColor`, `AppBorderWidth`, and `AppBorderRadius`. Draw the inset outline after content; never enlarge layout padding to accommodate it. Clamp painted radius to half the window's smaller dimension. If any one of those three fields is authored, every platform disables system window material so Go UI can paint the outline. Windows clips every custom-chrome window with a DPI-scaled region and composition clip, using 14 logical units when the radius is omitted, and updates the shape on resize/DPI changes; macOS detaches Liquid Glass/vibrancy and clips the renderer; Linux turns compositor blur off and can then paint rounded corners. Nil geometry preserves previous behavior; explicit zero stays zero and still disables system material. Theme previews use authored outline and radius values.
...
Launcher result alias and hotkey chips reuse shared keycaps in the explicit Dense variant: 16 logical-unit height/minimum width, 8 total horizontal inset, and 4-unit key gaps at normal density. Their type uses `TailFontSize` (11) so they match result tail tags. Scale their geometry and typography together with launcher density; preserve the existing Toolbar and Action Panel keycap geometry.
