...
Unless the user explicitly requests custom app/window corners, omit `AppBorderRadius` at the root and all platform/variant levels. Do not infer a custom window radius from a reference screenshot or a general request for a rounded visual style. Keep the system/default window shape and use a slightly translucent `AppBackgroundColor` by default (for example, alpha 0.90).

If the user explicitly requests custom app/window corners, set the requested `AppBorderRadius` and default `AppBackgroundColor` to fully opaque (alpha 1). An explicit request for transparency or opacity overrides either default. QueryBox, result, preview, and Action Panel corner settings do not count as a request for custom window corners. Toolbar and Action Panel may retain modest in-app translucency in either case.
...
- `AppBorderColor`, `AppBorderWidth`, and `AppBorderRadius` control launcher outer chrome. Width/radius use logical units; zero disables the outline or makes corners square. Omitted geometry keeps the previous platform behavior. Set width explicitly when designing an outline. Windows custom corners use a DPI-scaled window region plus a DirectComposition root clip, and disable System Backdrop, Accent blur, and extended glass frames. They retain transparency without native blur; use alpha below 1 in the app and surface backgrounds. macOS updates the material mask and glass radius. Linux with native blur retains square corners to avoid rectangular blur leaking outside the painted wash. Verify native material/shadow appearance on each target OS before promising identical results.

## Explain the custom-corner tradeoff

When creating, editing, or recommending a theme with an effective `AppBorderRadius` override on Windows, explicitly tell the user that custom corners disable the system material's translucent blur (Acrylic/desktop blur). The native window switches to transparent composition rather than system material; theme-color alpha controls ordinary window transparency. This does not disable Wox's own in-app frosted-glass surfaces. Include this limitation in the delivery message, not only in JSON or internal notes.

Toolbar and Action Panel retain their application-rendered frosted-glass transparency independently of the native window material. When designing custom-corner themes, consider modest transparency in `ToolbarBackgroundColor` and `ActionContainerBackgroundColor` (for example, alpha around 0.88–0.90), while keeping text readable. Do not make these surfaces opaque merely because custom window corners are enabled. An opaque `AppBackgroundColor` can remain intentional: these panels reveal or blur underlying app content, not the desktop through that opaque background.

An explicit `AppBorderRadius: 0` also enables this custom-shape path. To restore the system material, remove the override or set it to `null` at every applicable root/platform/variant level so the effective value is unset. Changing the radius to the apparent system default does not restore the material. Background colors still need alpha below 1 to reveal anything underneath.

Suggested user-facing wording: “Windows 下启用自定义圆角后，无法使用系统材质的模糊透明效果；窗口改用普通透明合成，透明程度由主题颜色的 Alpha 决定。Toolbar 和 Action Panel 的应用内磨玻璃效果仍保留，可以适度保留透明度。若要恢复系统材质，需要移除自定义圆角配置。”

This limitation describes the current Windows implementation. Do not present it as a universal macOS/Linux restriction; explain those platforms' behavior separately when relevant.
