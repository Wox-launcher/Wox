---
name: wox-align-flutter-ui
description: Align Wox Go UI screens, behavior, and interactions with the archived Flutter implementation through source-level contract tracing by default, then implement focused Go UI changes and run targeted tests. Use for Flutter-to-Go UI parity work, Settings or launcher synchronization, visual or interaction alignment against the last Flutter UI snapshot, and requests to make Go UI match the former Flutter product. Request a serial runtime UI comparison only for complex flows, native lifecycle behavior, ambiguous source contracts, or special visual effects that source inspection cannot establish reliably.
---

# Align Wox from Flutter UI

Align one named screen, component, state, or flow at a time. Use the archived Flutter source as the default product contract and current Go UI source as the implementation target. Source-level alignment is sufficient unless the user explicitly asks for runtime visual validation or the escalation criteria below apply.

## Flutter reference revision

Flutter UI was removed from `master` in `5c7d75a2c` (`feat: remove flutter ui`). Always treat the last commit that still contains `wox.ui.flutter` as the Flutter contract source:

- **Commit**: `dc1b4a0d92aefb00dd212bca223030a3657ebd64`
- **Short**: `dc1b4a0d9`
- **Subject**: `feat(ui): integrate embedded Go UI with core functionality`

Do not search Flutter UI on current `master`. Prefer this pinned revision for `git grep`, `git show`, and ownership mapping. If a path is missing at that revision, fall back to that path's own history with `git log` / `git rev-list`, and record the exact revision used.

Define a shell helper once per session so later commands stay short:

```bash
FLUTTER_UI_REF=dc1b4a0d9
```
...
Use source-level alignment when Flutter code at `$FLUTTER_UI_REF` can establish the contract for:
...
- an installed Flutter bundle may disagree with `$FLUTTER_UI_REF`
...
3. Inspect the working tree and references without modifying user work:
   ...
   git rev-parse --verify $FLUTTER_UI_REF
   ```
...
5. Never switch branches in a dirty checkout, stash user changes, discard changes, or overwrite unrelated work. Do not edit Flutter code unless explicitly requested. Flutter lives only in history; do not recreate `wox.ui.flutter` on `master` as part of alignment.
...
```bash
git grep -n '<visible text, widget, route, controller, or setting>' $FLUTTER_UI_REF -- wox.ui.flutter/wox
git show $FLUTTER_UI_REF:<path>
```
...
- Flutter route, view/widget, controller, and shared component owners at `$FLUTTER_UI_REF`
- Go UI window/controller, adapter, view, widget, and theme owners on current `master`
...
1. Format all changed files. Use `gofmt` for Go; use the repository-configured Dart line length only when Flutter edits were explicitly requested.
...
Capture Flutter first and Go UI second into separate directories under `/tmp/wox-ui-parity/<timestamp>/`. Record observable geometry and behavior rather than impressions. If an installed Flutter bundle disagrees with `$FLUTTER_UI_REF`, report the drift and treat the installed product as the runtime reference for that comparison.
...
- Flutter source revision (`dc1b4a0d9` or any path-specific fallback) and Flutter-to-Go ownership mapping
