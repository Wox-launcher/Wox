...
- **Data Migrations**: Implement persisted data and settings compatibility upgrades in `wox.core/migration/`, following its `README.md`: one registered, ordered migration per file, executed by the migration runner. Do not perform migration writes from plugin initialization, configuration loading, query handlers, or UI code. Preserve unrelated fields, make transformations safe to repeat, and follow existing transaction and Cloud Sync oplog conventions, including local-only settings. Runtime compatibility parsing may remain where needed, but must not rewrite stored data.

- **Public Plugin/SDK API Compatibility**: The context + named option struct + named result struct convention applies **only to Wox APIs exposed to third-party plugins through the plugin API/SDK contract**, including registration and unregistration APIs: `Screenshot(ctx context.Context, option ScreenshotOption) ScreenshotResult`. Put operation inputs other than context in the option struct and outputs (including success status when needed) in the result struct. Evolve these contracts by adding backward-compatible fields with defaults that preserve existing behavior, and keep the corresponding SDK contracts consistent. Do not break already published signatures merely to retrofit this convention; preserve existing entry points such as `GetCacheFolder(ctx context.Context) string`.
...
- **SVG Theme Colors**: For SVG icon paints that should adapt to Wox appearance, explicitly use `fill="var(--wox-theme-icon-color)"` or `stroke="var(--wox-theme-icon-color)"`. The shared launcher image pipeline resolves this variable to white (`#ffffff`) in dark themes and black (`#000000`) in light themes. Preserve fixed brand colors and gradients; do not infer theme behavior from authored black/white paints or recolor entire mixed-color SVGs. Standard SVG `currentColor` is not the Wox theme variable and retains its normal semantics. Existing controls that intentionally tint an entire icon may keep that behavior.
