...
IconOverlayAttachment IconOverlayCreateView(
    char* name,
...
void IconOverlayDestroyView(void* view);
*/
import "C"
import (
	"math"
...
func newIconRenderer(opts Options) (*iconRenderer, bool) {
	...
	var result C.IconOverlayAttachment
	mainthread.Call(func() {
		...
		result = C.IconOverlayCreateView(
			cName,
			cIconData,
			C.int(len(pngBytes)),
	...
	return &iconRenderer{handle: uintptr(result.handle), width: float64(result.width), height: float64(result.height)}, true
}
...
func (renderer *iconRenderer) destroy() {
	...
	mainthread.Call(func() {
		C.IconOverlayDestroyView(unsafe.Pointer(renderer.handle))
	})
	renderer.handle = 0
}
