...
var (
	linuxPortalMu        sync.Mutex
	linuxClipboardPortal linuxPortalClipboard
	linuxPortalCounter   uint64
)

func portalReady() error {
	linuxPortalMu.Lock()
	defer linuxPortalMu.Unlock()
	return ensureLinuxPortalClipboardLocked()
}

func portalReadContentType() Type {
	linuxPortalMu.Lock()
	defer linuxPortalMu.Unlock()
...
func portalReadText() (string, error) {
	linuxPortalMu.Lock()
	defer linuxPortalMu.Unlock()
	if err := ensureLinuxPortalClipboardLocked(); err != nil {
		return "", err
...
func portalReadFilePaths() ([]string, error) {
	linuxPortalMu.Lock()
	defer linuxPortalMu.Unlock()
	if err := ensureLinuxPortalClipboardLocked(); err != nil {
		return nil, err
...
func portalWriteFilePaths(filePaths []string) error {
	...
	return setLinuxPortalSelectionLocked(
		...
		map[string][]byte{portalMimeURIList: payload},
	)
}

func portalWriteImageBytes(pngData []byte) error {
	linuxPortalMu.Lock()
	defer linuxPortalMu.Unlock()
	if err := ensureLinuxPortalClipboardLocked(); err != nil {
		return err
	...
	return setLinuxPortalSelectionLocked(
		...
		map[string][]byte{portalMimePNG: pngData},
	)
}

func portalIsChanged() bool {
	linuxPortalMu.Lock()
	defer linuxPortalMu.Unlock()
	if err := ensureLinuxPortalClipboardLocked(); err != nil {
		return false
...
func portalWatchSnapshot() string {
	linuxPortalMu.Lock()
	defer linuxPortalMu.Unlock()
	if err := ensureLinuxPortalClipboardLocked(); err != nil {
		return fmt.Sprintf("backend=portal error=%s", err.Error())
	}
	return fmt.Sprintf("backend=portal type=%s mimes=%s", linuxClipboardPortal.latest.contentType, strings.Join(linuxClipboardPortal.latest.mimeTypes, ","))
}
...
func logLinuxPortalNoDataLocked(err error) {
	now := time.Now()
	if !linuxClipboardPortal.lastNoData.IsZero() && now.Sub(linuxClipboardPortal.lastNoData) < linuxClipboardNoDataLogInterval {
		return
	}
