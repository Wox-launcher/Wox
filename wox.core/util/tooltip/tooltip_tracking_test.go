...
func TestTooltipTrackingCloseWaitsForCursorToEnter(t *testing.T) {
	shouldClose, seenInside := tooltipTrackingClose(false, false, false, true, 0)
	...
	shouldClose, seenInside = tooltipTrackingClose(false, true, true, true, 0)
	if shouldClose || !seenInside {
		t.Fatalf("close=%v seen=%v, want to arm tracking after the cursor enters", shouldClose, seenInside)
	}

	shouldClose, seenInside = tooltipTrackingClose(true, false, false, true, 0)
	if !shouldClose || !seenInside {
		t.Fatalf("close=%v seen=%v, want to dismiss after the cursor leaves", shouldClose, seenInside)
	}

	shouldClose, seenInside = tooltipTrackingClose(false, false, false, false, tooltipOwnerLeaveGrace)
	if shouldClose || seenInside {
...
func TestTooltipTrackingCloseDismissesLeftoverAfterOwnerMove(t *testing.T) {
	shouldClose, seenInside := tooltipTrackingClose(false, false, true, true, 0)
	if shouldClose || seenInside {
		t.Fatalf("close=%v seen=%v, want to keep the first post-show samples that miss the trigger", shouldClose, seenInside)
	}

	shouldClose, seenInside = tooltipTrackingClose(false, false, true, true, tooltipOwnerLeaveGrace)
	if !shouldClose || seenInside {
	...
	shouldClose, seenInside = tooltipTrackingClose(false, false, false, true, tooltipOwnerLeaveGrace+time.Second)
	if shouldClose || seenInside {
