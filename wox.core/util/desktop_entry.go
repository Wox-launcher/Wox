...
func BuildLinuxDesktopEntry(includeURLField bool, autostart bool) (string, error) {
	...
	lines := []string{
		...
		"Categories=Utility;",
		"MimeType=" + pluginPackageURLMIME + ";" + PluginPackageMIMEType + ";" + ThemePackageMIMEType + ";",
		"Terminal=false",
...
func buildLinuxPluginPackageMimeType() string {
	return strings.Join([]string{
		...
		`  </mime-type>`,
		`  <mime-type type="` + ThemePackageMIMEType + `">`,
		`    <comment>Wox Theme Package</comment>`,
		`    <glob pattern="*` + ThemePackageExtension + `"/>`,
		`  </mime-type>`,
		`</mime-info>`,
		``,
	}, "\n")
