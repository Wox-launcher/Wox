...
func TestFileMediaHandlerSupportsRangeRequests(t *testing.T) {
	dir := t.TempDir()
	...
	NewFileMediaHandler().ServeHTTP(response, request)
...
func TestPreviewFileMediaContentType(t *testing.T) {
	...
	for filePath, want := range tests {
		t.Run(filePath, func(t *testing.T) {
			if got := previewFileMediaContentType(filePath); got != want {
