...
func (c *ExplorerPlugin) startOverlayListener(ctx context.Context) {
	...
	go func() {
		...
		buildExplorerShowContext := func(localCtx context.Context, rect common.WindowRect) (common.ShowContext, bool) {
			if rect.Width <= 0 || rect.Height <= 0 {
				c.typeToSearchDebugLog(localCtx, "showOverlay skipped (invalid rect w=%d h=%d)", rect.Width, rect.Height)
				return common.ShowContext{}, false
			}
			c.typeToSearchDebugLog(localCtx, "showOverlay explorerRect=(%d,%d,%d,%d)", rect.X, rect.Y, rect.Width, rect.Height)
			woxSetting := setting.GetSettingManager().GetWoxSetting(localCtx)
			initialWindowHeight := getExplorerInitialWindowHeight(localCtx)
			position := getExplorerWindowPosition(rect, woxSetting.AppWidth.Get()/2, initialWindowHeight)
		...
		resolveExplorerShowContext := func(localCtx context.Context) (common.ShowContext, bool) {
			...
			return buildExplorerShowContext(localCtx, common.WindowRect{X: x, Y: y, Width: w, Height: h})
		}
		...
		openDialogQuery := func(localCtx context.Context, pid int) {
			...
			showContext, ok := buildExplorerShowContext(localCtx, common.WindowRect{X: x, Y: y, Width: w, Height: h})
			if !ok {
				return
			}

			overlay.Close(explorerDialogHintOverlayName)
			// Seed the dialog owner before the secondary instance query is created,
			// so QueryEnv can resolve the open/save dialog as the source window.
