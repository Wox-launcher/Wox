...
func TestCompatibilityCorpus(t *testing.T) {
	...
	for _, tc := range []struct{ input, raw string }{
		...
		{"square root of 625", "25"}, {"2 power 10", "1024"}, {"4 power 6", "4096"}, {"60 + 74", "134"},
...
func TestInvalidAndResourceLimits(t *testing.T) {
	c, env := fixture()
	for _, input := range []string{"(1 USD + 2 USD", "1 USD +", "1 USD / 0", "1 USD + 2 meters", "1btc + 1", "1km/h in m", "2 meters^2 in ft", "2^100000", "(2^4096)^4096", "sqrt(-1)", "sin(1;2)", "1USD to EUR junk", "2026-03-29 02:30 Europe/Berlin to UTC", "2026-10-25 02:30 Europe/Berlin to UTC", "2026-02-30 2 am ist to cet", "time in nowhere-invalid", strings.Repeat("(", 65) + "1" + strings.Repeat(")", 65), strings.Repeat("9", 4097)} {
...
func TestLocaleAndPresentation(t *testing.T) {
	...
	if got := c.Format(r, FormatOptions{}).Formatted; got != "2 h 25 min" {
		t.Fatal(got)
	}
...
func TestTypeMatrix(t *testing.T) {
	...
	for _, tc := range []struct{ input, raw string }{
		...
		{"sqrt(4 meters^2)", "2 m"}, {"0.1 USD + 0.2 USD", "0.3 USD"},
	} {
		t.Run(tc.input, func(t *testing.T) {
			q, e := c.Parse(tc.input, ParseOptions{})
			if e != nil {
