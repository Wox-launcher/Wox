...
const (
	...
	// UI renders it from current settings so core does not duplicate UI-only hotkeys.
	WoxPreviewTypeHotkeyOverview = "hotkey_overview"
	// dictation_history carries both the refined output and its original ASR
	// transcript so UI can render a readable comparison instead of a quote.
	WoxPreviewTypeDictationHistory = "dictation_history"
	// media carries now-playing metadata for UI's animated record preview.
