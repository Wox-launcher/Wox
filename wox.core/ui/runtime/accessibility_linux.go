...
func updateLinuxAccessibility(window *platformWindow, tree AccessibilityTree) error {
	...
	for _, node := range tree.Nodes {
		...
		result := C.wox_linux_accessibility_add_node(
			native,
			C.uint64_t(node.ID),
...
func woxGoLinuxAccessibilityAction(context C.uintptr_t, nodeID C.uint64_t, action *C.char, value *C.char) C.int32_t {
