...
func TestPluginCommandsUseHintAndReadonlyTable(t *testing.T) {
	...
	rendered := state.Build(woxwidget.StateContext{}, grid.Widget).(woxwidget.Stack).Children[0].Child.(woxwidget.Flex)
	...
	firstCommand := body.Children[0].(woxwidget.Container).Child.(woxwidget.Flex).Children[0].(woxwidget.Container).Child.(woxwidget.Align).Child.(woxwidget.TextBlock).Value
	if firstCommand != "fix" {
