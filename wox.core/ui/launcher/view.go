...
func (a *App) buildLauncher(frame woxui.FrameInfo) woxwidget.Widget {
	...
	return woxwidget.Semantics{
		Key:          "launcher-window-key",
		AutomationID: "launcher.window",
		Role:         woxui.AccessibilityRoleWindow,
		Label:        "Wox",
...
func (a *App) buildQuery(snapshot viewSnapshot, width, height float32) woxwidget.Widget {
	...
	editor = woxwidget.EditableText{
		...
		Autofocus:    true,
		Disabled:     !queryFocused,
		OnKey:        a.onKey,
		OnTextInput: func(event woxui.TextInputEvent) bool {
			a.onTextInput(event)
			return true
		},
		OnSetValue: a.setQueryText,
		TextInput: func(bounds woxui.Rect) woxui.TextInputState {
			focus := max(0, min(len([]rune(snapshot.editing.Text)), snapshot.editing.Selection.Focus))
			metrics, _ := a.window.MeasureText(string([]rune(snapshot.editing.Text)[:focus]), style)
			return woxui.TextInputState{Enabled: true, CursorRect: woxui.Rect{X: bounds.X + metrics.Size.Width, Y: bounds.Y, Width: 1, Height: bounds.Height}}
		},
		Child: editor,
	}
...
func (a *App) buildResults(snapshot viewSnapshot, width, height float32) woxwidget.Widget {
	...
	rows := make([]woxwidget.Widget, 0, end-start)
	for index := start; index < end; index++ {
		...
		resultKey := woxwidget.Key(fmt.Sprintf("launcher-result-key-%s", result.ID))
		resultControl := woxwidget.Gesture{
			ID: fmt.Sprintf("result-gesture-%s", result.ID),
		...
		rows = append(rows, woxwidget.Semantics{
			Key:          resultKey,
			AutomationID: "launcher.result." + result.ID,
			Role:         woxui.AccessibilityRoleListItem,
			Label:        result.Title,
	...
	visiblePadding := containerPadding
	visiblePadding.Top += float32(start) * (rowHeight + resultRowGap)
	content := woxwidget.Container{Width: width, Height: contentHeight, Padding: visiblePadding, Child: woxwidget.Flex{Axis: woxwidget.Vertical, Gap: resultRowGap, Children: rows}}
	return woxwidget.Semantics{
		Key:          "launcher-results-key",
		AutomationID: "launcher.results",
		Role:         woxui.AccessibilityRoleList,
		Label:        "Search results",
		Child:        a.buildResultScrollSurface(content, snapshot.palette, width, height, contentHeight, offset),
	}
}
