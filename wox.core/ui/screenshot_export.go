...
// shares the same woxDataDirectory policy instead of letting UI guess where screenshots belong.
func reserveScreenshotExportFilePath() (string, error) {
	...
	for suffix := 0; ; suffix++ {
		...
		// concurrent capture cannot race UI into the same timestamp-based filename.
