...
<template>
  <section class="wox-section wox-quotes">
    <div class="wox-section-heading">
      <h2>{{ isZh ? "大家怎么说" : "What people are saying" }}</h2>
    </div>
    <div class="wox-quotes-grid">
      <article v-for="quote in quotes" :key="quote.text" class="wox-quote-card">
        <blockquote>
          <p>{{ quote.text }}</p>
        </blockquote>
