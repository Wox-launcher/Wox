...
`argument` 可以声明可选的 `Suggestions: string[]`，例如：
...
参数为空时，输入框内显示 `created / assigned / search`；输入 `cr` 后，显示灰色的
`eated` 和 Tab 标记。Tab 只补上剩余文字，不增加空格、不执行命令，也不跳到下一参数；
再次 Tab 沿用参数跳转。补全可一次撤销，未接受的提示不会进入参数值或复制内容。
匹配忽略大小写并按声明顺序取首项；已有完整匹配时不继续补全。候选值不做 i18n 翻译，
未匹配的文字仍可自由输入。仅在输入框聚焦、没有选区或输入法组合输入、光标位于参数末尾时补全。

明确触发词没有显式 Query Hint 时，Wox 从该插件当前的静态和动态 Commands 自动生成候选，
按声明顺序显示去重后的主命令名。例如 `gh ` 提示 `issues / prs`；别名仍参与命令解析，
但不列入候选。显式提示（包括普通占位符）完全优先；清除运行时提示后允许自动回退。
不为全局 `*` 插件聚合候选，也不装饰存在归属冲突的触发词。

插件元数据可声明 `"Features": [{"Name": "disableAutoCommandHint"}]`，
单独关闭该插件自动生成的命令提示；不声明时默认开启。显式声明的触发词和命令 Query Hint 不受影响，命令仍可正常调用。
Clipboard 使用此开关，让空查询专注于历史内容搜索。

空输入时最多显示三个完整候选，超出部分用 ` / …` 表示；宽度不足时进一步减少数量。
自动命令提示优先显示较短的命令（按 Unicode 字符数），同长度保留声明顺序。
这只影响展示，补全仍从全部候选中按原声明顺序匹配。
Tab 接受自动命令候选时会追加一个空格，并立即进入该命令的参数提示，整个操作可一次撤销；
插件声明的普通参数候选不自动加空格。

只有一个命令候选时，输入触发词和空格后直接显示完整的灰色命令名及 Tab 标记，
不再显示占位背景，也不必先输入首字母。Tab 接受后仍追加空格。
普通参数即使只有一个候选，空值时仍保持占位提示。

补全 `issues` 后输入空格，会切换到该命令声明的 Query Hint。
现有注册和 ChangeQuery 接口均可携带 `Suggestions`；未声明该字段时保留原有提示行为。
