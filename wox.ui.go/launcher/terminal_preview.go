...
func (a *App) reconcileTerminalSubscription() {
	...
	if a.terminalSubscribed != "" {
		if _, err := a.client.SendRequest("TerminalUnsubscribe", map[string]any{"sessionId": a.terminalSubscribed}); err != nil {
	...
	if _, err := a.client.SendRequest("TerminalSubscribe", map[string]any{"sessionId": desiredSessionID, "cursor": -1}); err != nil {
...
func (a *App) requestTerminalHistory(sessionID string, cursor int64) {
	...
	if current && a.terminalSubscribed == sessionID {
		sent = true
		_, err = a.client.SendRequest("TerminalSubscribe", map[string]any{"sessionId": sessionID, "cursor": cursor})
	}
