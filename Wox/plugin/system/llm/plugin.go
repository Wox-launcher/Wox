...
func (c *Plugin) getDynamicSetting(ctx context.Context, key string) definition.PluginSettingDefinitionItem {
	if key == "dynamic_models" {
		...
		for _, model := range c.getProviderModels(ctx, modelProviderName(provider)) {
...
func (c *Plugin) loadClient(ctx context.Context) {
	...
	c.provider = provider
	c.api.Log(ctx, plugin.LogLevelInfo, fmt.Sprintf("chatgpt provider created with model: %s", modelName))
}
