...
func (c *BrowserBookmarkPlugin) Query(ctx context.Context, query plugin.Query) []plugin.QueryResult {
	var filter = lo.Filter(c.bookmarks, func(bookmark Bookmark, _ int) bool {
		return plugin.GetPluginManager().StringMatch(ctx, bookmark.Name, query.Search) ||
			plugin.GetPluginManager().StringMatchNoPinYin(ctx, bookmark.Url, query.Search)
	})
	return lo.Map(filter, func(bookmark Bookmark, _ int) plugin.QueryResult {
