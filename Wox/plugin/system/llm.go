		...
		SettingDefinitions: definition.PluginSettingDefinitions{
			{
				...
				Value: &definition.PluginSettingValueSelect{
					...
					DefaultValue: string(llm.ModelProviderNameOpenAI),
					Options: []definition.PluginSettingValueSelectOption{
						{
							Label: "OpenAI",
							Value: string(llm.ModelProviderNameOpenAI),
						},
						{
							Label: "Google",
							Value: string(llm.ModelProviderNameGoogle),
						},
						{
							Label: "Ollama",
							Value: string(llm.ModelProviderNameOllama),
						},
						{
							Label: "Groq",
							Value: string(llm.ModelProviderNameGroq),
						},
					},
...
func (c *Plugin) Query(ctx context.Context, query plugin.Query) []plugin.QueryResult {
	if query.Type == plugin.QueryTypeSelection {
		if !llm.IsInstanceReady() || query.Selection.Type == util.SelectionTypeFile {
	...
	if !llm.IsInstanceReady() {
		return []plugin.QueryResult{
			{
				Title:    "LLM setting not initialized",
...
func (c *Plugin) getDynamicSetting(ctx context.Context, key string) definition.PluginSettingDefinitionItem {
	if key == "dynamic_models" {
		...
		for _, m := range c.getProviderModels(ctx, llm.ModelProviderName(provider)) {
	...
	if key == "dynamic_host" {
		if c.api.GetSetting(ctx, "provider") == string(llm.ModelProviderNameOllama) {
			return definition.PluginSettingDefinitionItem{
				Type: definition.PluginSettingDefinitionTypeTextBox,
				Value: &definition.PluginSettingValueTextBox{
					Key:          "host_" + string(llm.ModelProviderNameOllama),
					Label:        "Host",
...
func (c *Plugin) getProviderModels(ctx context.Context, providerName llm.ModelProviderName) (models []llm.Model) {
	if provider, providerErr := llm.NewProvider(ctx, llm.ProviderConnectContext{
...
func (c *Plugin) loadClient(ctx context.Context) {
	...
	provider, providerErr := llm.NewProvider(ctx, llm.ProviderConnectContext{
		Provider: llm.ModelProviderName(providerName),
	...
	currentProvider, currentModel := llm.GetInstance()
	if currentProvider != nil {
		closeErr := currentProvider.Close(ctx)
		if closeErr != nil {
			c.api.Log(ctx, plugin.LogLevelError, fmt.Sprintf("failed to close llm provider: %s", closeErr.Error()))
		} else {
			c.api.Log(ctx, plugin.LogLevelInfo, fmt.Sprintf("llm provider closed, model: %s", currentModel.Name))
		}
	}

	var availableModel llm.Model
	models := c.getProviderModels(ctx, llm.ModelProviderName(providerName))
	...
	llm.SetInstance(provider, availableModel)
...
func (c *Plugin) queryCommand(ctx context.Context, query plugin.Query) []plugin.QueryResult {
	...
	for index, message := range prompts {
		...
		if index%2 == 0 {
			...
			conversations = append(conversations, llm.Conversation{
				Role: llm.ConversationRoleSystem,
				Text: msg,
			})
		}
	}
	...
	_, model := llm.GetInstance()
	return []plugin.QueryResult{{
		Title:           fmt.Sprintf("Chat with %s", query.Command),
		SubTitle:        fmt.Sprintf("%s - %s", model.Provider, model.DisplayName),
