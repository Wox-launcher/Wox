...
func (c *Plugin) Query(ctx context.Context, query plugin.Query) []plugin.QueryResult {
	if query.Type == plugin.QueryTypeSelection {
		//TODO: maybe support ai commands for files in the future
				...
				Actions: []plugin.QueryResultAction{
					{
						...
						Action: func(ctx context.Context, actionContext plugin.ActionContext) {
							c.api.Log(ctx, plugin.LogLevelInfo, fmt.Sprintf("run tool: %s", command.Command))
							c.api.ChangeQuery(ctx, share.PlainQuery{
								QueryType: plugin.QueryTypeInput,
								QueryText: fmt.Sprintf("%s %s %s", c.GetMetadata().TriggerKeywords[0], command.Command, query.Selection.Text),
							})
						},
					},
				},
