...
namespace Wox.Plugin.Program.Programs
{
    ...
    public class UWP
    {
        ...
        private void InitPackageVersion(string[] namespaces)
        {
            ...
            ProgramLogger.LogException($"{Location}" +
        ...
        public static Application[] All()
        {
            ...
            if (support)
            {
                var applications = CurrentUserPackages().AsParallel().SelectMany(p =>
                {
                    ...
                    {
                        ProgramLogger.LogException($"An unexpected error occured and "
        ...
        public class Application : IProgram
        {
            ...
            internal string ResourceFromPri(string packageFullName, String name, string resourceReference)
            {
                ...
                if (!string.IsNullOrWhiteSpace(resourceReference) && resourceReference.StartsWith(prefix))
                {
                    ...
                    if (hResult == Hresult.Ok)
                    {
                        ...
                        {
                            ProgramLogger.LogException($"Can't load null or empty result "
                    ...
                    else
                    {
                        ...
                        var e = Marshal.GetExceptionForHR((int)hResult);
                        ProgramLogger.LogException($"Load pri failed {source} with HResult {hResult} and location {Package.Location}", e);
                        return string.Empty;
                    }
                }
                else
            ...
            internal string LogoPathFromUri(string uri)
            {
                ...
                if (extension != null)
                {
                    ...
                    {
                        ProgramLogger.LogException($"{Package.Location}" +
