...
class PdfFilePreviewRenderer implements WoxFilePreviewRenderer {
  ...
  WoxFilePreviewResult render(WoxFilePreviewContext context) {
    if (context.filePath.startsWith("http")) {
      return WoxFilePreviewResult(content: _buildPdfWebView(context.filePath, cacheDisabled: true), contentHandlesScrolling: true);
    }
    ...
    return WoxFilePreviewPolicy.buildDeferredPreview(
      ...
      typeLabel: typeLabel,
      previewKey: cacheKey,
      previewBuilder: (_) => _buildPdfWebView(buildFilePreviewMediaSource(file), cacheKey: cacheKey),
    );
  }

  Widget _buildPdfWebView(String source, {String cacheKey = "", bool cacheDisabled = false}) {
    final previewData = WoxPreviewWebviewData(url: source, cacheKey: cacheKey, cacheDisabled: cacheDisabled);
    return WoxWebViewPreview(previewData: jsonEncode(previewData.toJson()), showToolbar: false);
  }
}
