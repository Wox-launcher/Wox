...
class FolderFilePreviewRenderer implements WoxFilePreviewRenderer {
  ...
  WoxFilePreviewResult render(WoxFilePreviewContext context) {
    ...
    try {
      ...
      return WoxFilePreviewResult(content: _FolderFilePreview(data: data, tr: context.tr), previewTags: _buildFolderPreviewTags(directory, data, context.tr));
    } catch (e) {
...
class _FolderFilePreview extends StatelessWidget {
  ...
  const _FolderFilePreview({required this.data, required this.tr});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        for (final entry in data.entries) _FolderEntryRow(entry: entry),
...
class _FolderEntryRow extends StatelessWidget {
  ...
  const _FolderEntryRow({required this.entry});
...
class _FolderPreviewEntry {
  ...
  const _FolderPreviewEntry({required this.name, required this.size, required this.isDirectory, required this.modified});
}
