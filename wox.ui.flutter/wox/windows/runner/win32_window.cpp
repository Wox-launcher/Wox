                         ...
                         const Size &size)
{
  ...
  // Apply native backdrop effect after HWND creation so Windows can attach the
  // compositor material to the real top-level window.
...
void Win32Window::ApplyBackdropForAppearance(bool use_dark)
{
  ...
  if (GetWindowsBuildNumber() >= kWindows11BuildNumber)
  {
    // Theme changes can reset the DWM material on the existing HWND. Reapply the
    // same Windows 11 backdrop policy used at creation instead of falling back to
    // an opaque frame until the next resize.
    MARGINS margins = {-1};
