...
void registerLauncherOnboardingSmokeTests() {
  group('T1B: Onboarding Smoke Tests', () {
    testWidgets('T1B-01: unfinished startup opens onboarding and skip finishes before launcher', (tester) async {
      ...
      expectOnboardingWindowOpen(controller);
      ...
      expect(controller.isOnboardingWindowOpen.value, isFalse);
    ...
    testWidgets('T1B-02: about page can reopen onboarding and complete returns to launcher', (tester) async {
      ...
      await pumpUntil(
        tester,
        () => controller.isOnboardingWindowOpen.value && find.byKey(const ValueKey('onboarding-view')).evaluate().isNotEmpty,
    ...
    testWidgets('T1B-03: onboarding config pages save immediately and glance never renders blank', (tester) async {
      ...
      expect(find.byKey(const ValueKey('onboarding-wpm-install-page')), findsOneWidget);
      expect(find.byKey(const ValueKey('onboarding-wpm-install-demo')), findsOneWidget);
      await _goNext(tester); // theme install
      expect(find.byKey(const ValueKey('onboarding-theme-install-page')), findsOneWidget);
      expect(find.byKey(const ValueKey('onboarding-theme-install-demo')), findsOneWidget);
    });
  });
}
