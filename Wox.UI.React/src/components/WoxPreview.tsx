  ...
  return <Style theme={WoxThemeHelper.getInstance().getTheme()}>
    <WoxScrollbar scrollbarProps={{ autoHeight: true, autoHeightMin: 0, autoHeightMax: props.resultSingleItemHeight * 8 + 10 }}>
      <div className={"wox-query-result-preview-content"}>
        {props.preview.PreviewType === WoxPreviewTypeEnum.WoxPreviewTypeText.code &&
          <p className={"wox-query-result-preview-text"}>{props.preview.PreviewData}</p>}
        {props.preview.PreviewType === WoxPreviewTypeEnum.WoxPreviewTypeImage.code &&
          <img className={"wox-query-result-preview-image"} src={props.preview.PreviewData} />}
        {props.preview.PreviewType === WoxPreviewTypeEnum.WoxPreviewTypeMarkdown.code &&
          <Markdown>{props.preview.PreviewData}</Markdown>}
        {props.preview.PreviewType === WoxPreviewTypeEnum.WoxPreviewTypeUrl.code &&
          <iframe className={"wox-query-result-preview-url"} src={props.preview.PreviewData}></iframe>}
      </div>
    </WoxScrollbar>
      ...
      <div className={"wox-query-result-preview-properties"}>
        {Object.keys(props.preview.PreviewProperties)?.map((key) => {
          return <div key={`key-${key}`} className={"wox-query-result-preview-property"}>
            ...
            <div
              className={"wox-query-result-preview-property-value"}>{props.preview.PreviewProperties[key]}</div>
          </div>
        })}
      </div>
    }
